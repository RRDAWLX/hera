...
function paramCheck (params, paramTpl) {
  ...
  var name =
      arguments.length > 2 && void 0 !== arguments[2]
        ? arguments[2]
        : 'parameter'
  var tplTpye = getDataType(paramTpl)
  var pType = getDataType(params)
...
function getRealRoute (pathname, url) {
  ...
  var pathArr = pathname.length > 0 ? pathname.split('/') : []
...
function isComponentExist (params) {
  ...
  var attribute = params[1] // 属性名
  var option = params[2] // 组件属性可选值
...
function isAPIExist (params) {
  ...
  var method = params[1] // 调用方式：有效值为return, success, object, callback
  var param = params[2] // 组件属性可选值
  var options = params[3]
  var methods = ['return', 'success', 'object', 'callback']
  var api = APIs[name]
  if (api) {
    if (!method) {
      return true
